    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          {t('dashboard.subtitle') || 'Обзор вашего производства сегодня'}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <StatCard
          ...
          value={`${(stats?.monthlyRevenue || 0).toLocaleString()} сом`}
          color="bg-green-500"
        />
        <StatCard
          icon={FiShoppingCart}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold text-gray-700">Выпуск продукции</h3>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={chartData} barGap={4}>
              ...
              <Bar dataKey="sewn" name="Сшито" fill="#3b82f6" radius={[6, 6, 0, 0]} />
              <Bar dataKey="packed" name="Упаковано" fill="#93c5fd" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
          <div className="flex items-center gap-6 mt-4 pt-4 border-t">
            <div>
              <p className="text-xs text-gray-500">Сшито за месяц</p>
              <p className="text-lg font-bold text-gray-800">{(stats?.totalSewn || 0).toLocaleString()} шт.</p>
            </div>
            <div>
              <p className="text-xs text-gray-500">Брак</p>
              <p className="text-lg font-bold text-red-500">{stats?.totalDefective || 0} шт.</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold text-gray-700">Последние заказы</h3>
